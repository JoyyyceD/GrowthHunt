...
function canonicalCheck(ctx: AuditContext): Check {
  ...
  if (!href) {
    return mkCheck('canonical', 'canonical 标签存在', 10, 'fail',
  ...
  try {
    ...
    return mkCheck('canonical', 'canonical 标签存在且自指', 10, sameOrigin ? 'pass' : 'partial',
      sameOrigin
        ? { detail: `Canonical: ${canon.pathname}` }
        : { detail: `Canonical points off-origin (${canon.origin})`, fix: 'Point the canonical URL at this page' })
  } catch {
    return mkCheck('canonical', 'canonical 标签存在', 10, 'partial',
...
function sitemapUrlCheck(ctx: AuditContext): Check {
  if (!ctx.sitemapXml.found) {
    return mkCheck('url-in-sitemap', '当前 URL 在 sitemap 内', 12, 'na',
  ...
  if (present) {
    return mkCheck('url-in-sitemap', '当前 URL 在 sitemap 内', 12, 'pass',
  ...
  return mkCheck('url-in-sitemap', '当前 URL 在 sitemap 内', 12, 'fail',
...
function llmsStructureCheck(ctx: AuditContext): Check {
  if (!ctx.llmsTxt.found) {
    return mkCheck('llms-structured', 'llms.txt 结构合规', 8, 'na', { detail: 'No llms.txt' })
  }
  ...
  return mkCheck('llms-structured', 'llms.txt 结构合规', 8, status,
    status === 'pass'
...
export const discovery: Dimension = {
  ...
  run(ctx: AuditContext) {
    ...
    checks.push(mkCheck('sitemap-exists', 'sitemap.xml 存在', 12,
    ...
    checks.push(mkCheck('llms-exists', '/llms.txt 存在', 14,
