...
## Competitive set
Markdown table covering EXACTLY the competitors from the intelligence — no additions from memory, even well-known ones; pricing cells only from the intelligence (write "—" when unknown): Product | Format | Pricing | Key difference vs ${intel.product.name} | Who it fits. Wide and shallow — the deep dive on the leader lives in competitor-deep-dive.md.
