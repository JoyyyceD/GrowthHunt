import { redirect } from 'next/navigation'
import type { Metadata } from 'next'
import { TopNav } from '@/lib/site/TopNav'
import { createServerClient } from '@/lib/supabase/server'
import { listWorkspacesForOwner, getWorkspace } from '@/lib/workspace/store'
import { IcpRunner } from './IcpRunner'

const TITLE = 'ICP / Positioning Agent — GrowthHunt'
const DESCRIPTION = 'Auto-draft your ICP, positioning statement, and key messages from a URL + brief. Saves back to your workspace.'

export const metadata: Metadata = { title: TITLE, description: DESCRIPTION }

export default async function IcpAgentPage({ searchParams }: { searchParams: Promise<{ ws?: string }> }) {
  const sp = await searchParams
  const supabase = await createServerClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) redirect('/login?next=/agents/icp')

  const workspaces = await listWorkspacesForOwner(user.id)
  if (workspaces.length === 0) redirect('/workspace')
  const active = sp.ws ? await getWorkspace(sp.ws) : workspaces[0]
  if (!active || (active.owner_id && active.owner_id !== user.id)) redirect('/workspace')

  return (
    <div>
      <TopNav variant="page" />
      <section style={{ padding: '48px 0' }}>
        <div className="shell" style={{ maxWidth: 880 }}>
          <div className="eyebrow"><span className="dot" />ICP Agent · beta</div>
          <h1 style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(32px, 4.5vw, 52px)', fontWeight: 400, letterSpacing: '-0.025em', margin: '12px 0 14px' }}>
            Who buys it, why, what to lead with.
          </h1>
          <p style={{ fontSize: 16, color: 'var(--ink-dim)', lineHeight: 1.6, margin: '0 0 28px', maxWidth: 580 }}>
            Reads your homepage + workspace context and drafts a tight ICP, positioning statement,
            key messages and competitor list. One click writes it back to the workspace; every
            downstream agent picks it up.
          </p>
          <IcpRunner workspace={active} allWorkspaces={workspaces} />
        </div>
      </section>
    </div>
  )
}
