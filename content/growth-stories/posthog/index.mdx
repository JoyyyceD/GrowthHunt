...
The second pattern is structurally distinct from anything else in the case set so far. PostHog publishes its handbook, compensation philosophy, salary calculator, runway, ARR, OKRs, cap table, and strategy on the open web at `posthog.com/handbook` — under MIT-licensed Markdown that anyone can submit a pull request against. <Stat value="6 years" label="of public handbook commits" /> make this un-fakeable.
...
PostHog publishes more than any other company in the case set, but transparency is not absolute. The opaque parts:
