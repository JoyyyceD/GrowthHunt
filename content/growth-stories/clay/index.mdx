...
A solo Series B announcement would have gotten Clay three to five days of coverage in capital press. A Series B + Series A retroactive + customer roster + user count + 2,500 paying-customer milestone got the same window across capital, dev, SaaS, and operator press. **Same announcement budget, four times the surface.** This is the cleanest C1 (bundled milestone) execution in our case set.

## The compound stack — what B1 plus B3 plus E2 actually means together

Most case studies in our case set use one or two of these moves. Clay is the first to weave all three into a single self-feeding engine. The compound table:
