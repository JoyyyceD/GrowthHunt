...
## 批次 8：V2 计划 ✅ 已敲定（2026-06-11）

详见 [agent-v2-plan.md](./agent-v2-plan.md)。核心决策：
- ✅ **8.1 V2 主线 = 获客优先**：分享报告页（公开只读 playbook + OG 卡片 + CTA）为核心抓手；公开 launch 放心跳之后；计费最后
- ✅ **8.2 心跳形态** = 晨报（Scout 主动消息）+ 草稿入 proposed 队列；节奏默认工作日每天、可调；连续不 approve 自动降频
- ✅ **8.3 顺序**：阶段0 v1收尾（案例引擎/Claude验收/gate）→ 分享报告页 → 心跳 → 日历/Activity → ROI 闭环 → 计费
- ✅ **8.4 D1-D5**：分享默认私有一键开启 / 心跳工作日可调 / 晨报站内先行邮件v2.5 / 定价内测后定 / 已分享报告页允许 SEO 收录
- 📋 外部约束记录：X 指标 API 需 Basic $200/月——ROI 闭环用 Reddit 公开数据 + UTM 点击代理起步
